...
namespace SDA_Project.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            if(A.loginEmail == string.Empty || A.adminPassword == string.Empty)
            {
                TempData["Message"] = "<script> alert('Please fill out the required fields!')  </script>";
            }
            else
            {
                //Singleton AdminSingleton = Singleton.GetInstance();
                conn.Open();
                ...
                cmd.Parameters.AddWithValue("Pass",A.adminPassword);
                SqlDataReader sdr = cmd.ExecuteReader();
                if (sdr.Read()) 
                {
